		...
		<!-- 头部 start-->
			...
			<input type="hidden" name="id" id="id" value="${cstCase.id }"/>
						...
						<div class="row center-block">
							...
							<div class="col-xs-4 text-center">
								<dl>
									 ...
									 <dd class="input-sm"><a class="btn btn-xs" href="${ctx }/cons/cstCase/picture?caseId=${cstCase.caseLib.id}&typed=1" target="_blank"><img alt="切片" src="${ctxStatic}/AdminLTE/mystatic/images/slice_button.png"></a></dd>
								</dl>
							</div>
						</div>
					</fieldset>
					...
					<fieldset style="padding:.15em .325em .5em;margin:0 2px;border:1px solid silver"> 
						...
						<div class="col-xs-12">
							<dl>
								...
								<dd class="input-sm">
									<div class="col-xs-12">
										<textarea rows="4"  class="form-control input-sm" name="cstDia" id="cstDia" disabled>${cstCase.cstDia }</textarea>
									</div>
								</dd>
							</dl>
						</div>
				...
				<div id="ex2" class="tab-pane fade center-block">
					...
					<caption class="bg-info">&nbsp;<button class="btn btn-xs btn-primary disabled" disabled><span class="glyphicon glyphicon-plus"></span>申请加做</button></caption>
						<thead>
							<tr>
								<th>序号</th>
								<th>加做内容</th>
				...
				<div id="ex3" class="tab-pane fade center-block">
					<div class="table-responsive"><!-- 响应式表格-->
						<table class="table table-bordered table-hover">
							<caption class="bg-info">&nbsp;<button class="btn btn-xs btn-primary disabled" disabled><span class="glyphicon glyphicon-plus"></span>添加反馈</button></caption>
							<thead>
								<tr>
									<th>序号</th>
									<th>留言</th>
	                    ...
	                    if(html.length>0||html=="[]"){
			                    ...
			                    for(var i=0;i<=html.length-1;i++) {
			                    	htmls = JSON.parse(html[i].image);
			                    		...
			                    		$("#box ul").append("<li><img  src='"+ctx+htmls[0].path+"' width='198px' height='98px' name='"+name+"' id='"+(i+1)+"'> </li>");
			                    		init();
			                    		var jsonData = {};
			                    		jsonData.name = name;
...
$.ajax({
    ...
    type:"post",
    dataType:"json",
	data:{testId:case_id,filePath:obj.name,fileName:obj.id,time:new Date().getTime()},
	success: function(html) {
    ...
    if(aImg && aImg.length > 0){
    	oFrame.innerHTML='<img src="'+aImg[0].src+'" id="'+aImg[0].id+'" /><span style="font-size: 15px;position: relative;bottom: -20px;right:-70px;">第1/'+(aImg.length)+'张</span><span style="font-size: 15px;position: relative;bottom: 310px;font-weight: bold;">'+aImg[0].name+'</span><span style="font-size: 15px;position: relative;bottom: 20px;right:-230px;"><input type="button" name='+aImg[0].src+' id='+aImg[0].name+' value="删除" onclick="doDel(this)"></span>';
    }
	}
	...
	$.ajax({
						...
						success : function(data) {
							for(n in data){
								...
								html+="<li><img  src='"+ctx+paths[0].path+"' width='198px' height='98px' name='"+ name +"' id='"+(parseInt(n)+1)+"'> </li>  ";
								var jsonData = {};
								jsonData.name = name;
